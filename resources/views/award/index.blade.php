...
@section('content')
    <div class="da-page container py-5">
        <div class="row">
            <div class="col-12 d-flex align-items-center justify-content-between">
                <h3 class="da-page__title">Premiações</h3>
                <a href="{{ route('award.create') }}" class="btn btn-primary font-weight-bold"
                    title="Cadastrar Premiação">Cadastrar Premiação</a>
            </div>
        </div>
        <div class="row">
                    ...
                    @if (isset($awards) && count($awards) > 0)
                        <table class="table da-table table-striped">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $award->name }}</td>
                                    <td>{{ date("d/m/Y", strtotime($award->start)) ?? "--" }}</td>
                                    <td>{{ date("d/m/Y", strtotime($award->end)) ?? "--" }}</td>
                                    <td>
                                        @if ($award->status == "enable")
                                    ...
                                    <td class="text-right">
                                        <a href="{{ route('rate.index', $award->uuid) }}" class="btn btn-secondary btn-sm" data-toggle="tooltip" title="Taxas">
                                        ...
                                        @if ($award->status == "enable")
                                        <button type="button" class="btn btn-warning btn-sm btn-status" data-uuid="{{ $award->uuid }}" data-status="disable" data-toggle="tooltip" title="Desabilitar">
                                        ...
                                        @else
                                        <button type="button" class="btn btn-success btn-sm btn-status" data-uuid="{{ $award->uuid }}" data-status="enable" data-toggle="tooltip" title="Ativar">
                                        ...
                                        @endif
                                        <a href="{{ route('award.edit', $award->uuid) }}" class="btn btn-primary btn-sm">
                                        ...
                                        <button type="button" class="btn btn-danger btn-sm delete" data-delete="{{ $award->uuid }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
    ...
    <form id="form_delete" method="post" action="{{ route('award.delete') }}">
        @csrf
        @method('delete')
        <input type="hidden" name="award" id="award_delete" value="">
    </form>
@endsection
            ...
            $('.delete').on('click', function(e){
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $('#form_delete').find('#award_delete').val(button.data('delete'));
                        $('#form_delete').submit();
                    }
                })
            });
