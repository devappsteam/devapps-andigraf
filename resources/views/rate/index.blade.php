    ...
    <div class="da-page container py-5">
        <div class="row">
            <div class="col-12 d-flex align-items-center justify-content-between">
                <h3 class="da-page__title">Taxas</h3>
                <button class="btn btn-primary font-weight-bold" title="Cadastrar Taxa" data-toggle="modal"
                    data-target="#modal_rate" data-action="create">Cadastrar Taxa</button>
            </div>
        </div>
        <div class="row">
                        ...
                        <table class="table da-table table-striped">
                            ...
                            <tbody>
                                @foreach ($rates as $rate)
                                    <tr>
                                        <td>{{ $rate->quantity }}</td>
                                        <td>R$ {{ number_format($rate->price, 2, ',', '.') }}</td>
                                        <td>R$ {{ number_format(($rate->quantity * $rate->price), 2, ',', '.') }}</td>
                                        <td class="text-right">
                                            <button class="btn btn-primary btn-sm btn-edit" data-uuid="{{ $rate->uuid }}"
                                                data-quantity="{{ $rate->quantity }}" data-price="{{ $rate->price }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Taxas da Premiação</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span>&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('rate.store', $request->award) }}" method="post" id="_form_rate">
                        @csrf
                        @method('post')
                        @include('rate.form')
                    </form>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" form="_form_rate">Salvar e Fechar</button>
                </div>
            </div>
        </div>
    </div>
