...
@section('content')
                ...
                <form method="get">
                                ...
                                @if (isset($associates))
                                    @foreach ($associates as $associate)
                                        <option value="{{ $associate->id }}"
                                            {{ isset($_GET['associate']) && $_GET['associate'] == $associate->id ? 'selected' : '' }}>
                                ...
                                @if (isset($awards))
                                    @foreach ($awards as $award)
                                        <option value="{{ $award->id }}"
                                            {{ isset($_GET['award']) && $_GET['award'] == $award->id ? 'selected' : '' }}>
                                            {{ $award->name }}
                                        </option>
                                    @endforeach
                                @endif
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <a href="{{ route('product.index') }}" class="btn btn-secondary">Redefinir</a>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="col-12 mt-4">
                <div class="da-box da-box--column table-responsive">
                    @if (isset($products) && count($products) > 0)
                        <table class="table da-table table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th>Evento</th>
                                    <th>Conclusão</th>
                                    <th width="120" class="text-right">Opções</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-truncate">
                                            @if ($product->associate->type == 'legal')
                                                {{ $product->associate->fantasy_name }}
                                            @else
                                                {{ $product->associate->first_name }}
                                            @endif
                                        </td>
                                        <td class="text-truncate">{{ $product->award->name }}</td>
                        ...
                        {{ $products->appends(request()->query())->links('layouts.partials.paginate') }}
                    @else
                        ...
                        <form action="{{ route('report.product.export') }}" method="post" id="form_export">
                            ...
                            <div class="row">
                                <div class="col-12 form-group">
                                    <label for="search">Pesquisar</label>
                                    <input type="text" name="search" id="search" class="form-control"
                                        placeholder="Nome do Produto" value="{{ $_GET['search'] ?? '' }}">
                                </div>
    ...
    @endif
    <form id="form_ticket" method="post" action="{{ route('product.ticket') }}" target="_blank">
        @csrf
        @method('post')
        <input type="hidden" name="search"
            value="{{ isset($_GET['search']) && !empty($_GET['search']) ? $_GET['search'] : '' }}">
        <input type="hidden" name="associate"
            value="{{ isset($_GET['associate']) && !empty($_GET['associate']) ? $_GET['associate'] : '' }}">
        <input type="hidden" name="category"
            value="{{ isset($_GET['category']) && !empty($_GET['category']) ? $_GET['category'] : '' }}">
        <input type="hidden" name="award"
            value="{{ isset($_GET['award']) && !empty($_GET['award']) ? $_GET['award'] : '' }}">
    </form>
@endsection
